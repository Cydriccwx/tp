...
| **Parser & Validation**      | Implemented parts of the **`Parser`** for command routing and **`ParserUtil`** for reusable data parsing and validation helpers (price parsing, ticker formatting, direction validation, stop loss validation). |
...
| **Documentation**             | Documented the new workflow, reorganised the User Guide feature list, and updated the Developer Guide diagrams for the ListCommand, UI classes as well as the  shortcut and compare features so the implementation remains fully traceable.               |
